...
void DashBoard::paintEvent(QPaintEvent* event) {
  this->resize(parent_->size());
  QWidget::paintEvent(event);

  int side = qMin(int(parent_->width() / 1.8), parent_->height());

  QPainter painter(this);
  painter.setRenderHint(QPainter::Antialiasing);
  painter.translate(parent_->width() / 2, parent_->height() / 2);
...
void DashBoard::draw_speedometer(QPainter& painter) {
  ...
  painter.drawText(QRect(80, 50, 70, 50), Qt::AlignBottom | Qt::AlignLeft,
                   QString("%0").arg(QString::number(speed_), 3, '0'));
...
void DashBoard::draw_gear(QPainter& painter) {
  ...
  switch (gear_) {
    case kGear_1:
      painter.drawText(gear_rect, Qt::AlignCenter, QString::number(gear_));
...
void DashBoard::draw_thermometer(QPainter& painter) {
  ...
  painter.rotate(-12 - 0.2 * temperature_);
